...
<META name=GENERATOR content="MSHTML 8.00.7601.17514">
  ...
  <script language="JavaScript">
    function isadmin(oname,opwd){
        var oname=$("#user_name").val();
        var opwd=$("#user_password").val();
        $.post("admlog",{"name":oname,"password":opwd},function (data) {
            if(data.result!=null){
                alert("success")
            }else{
                $.messager.alert('提示信息','不是管理员','error');
            }
        },"json");
    }
  </script>
  ...
  <TR>
    ...
    <TD><!-- <input type="password" class="text" name="password" /> --><INPUT 
      id=user_password class=text type=password name=password> </TD></TR><!--
						...
						--></TBODY></TABLE>
<DIV class=buttons><INPUT onclick="isadmin(oname,opwd)" value="管理员登录" type=button id="adm"></DIV>
